# Since the 'BeautifulSoup' module is not available, we can use Python's built-in string manipulation functions to process the webpage content.
...
html = urlopen(url)
webpage_content = html.read().decode('utf-8')
